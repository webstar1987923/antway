...
export class ViolationListComponent implements OnInit, OnDestroy {
	queryParams: any = {
		pi: 1,
		ps: 10,
		active: '',
	...
	columns: STColumn[] = [
		...
		},
		{
			title: '操作员工',
			index: 'admin',
			format: (item, _col) => `${item.admin.user_name}`,
		},
		{ title: '违规时间', type: 'date', index: 'created_at', sort: true },
	];
	selectedRows: STData[] = [];
	totalCallNo = 0;
	...
	getData() {
		...
		this.comSrv
			.getCompanyViolations({
				...this.queryParams,
				company_id: this.data.record.id,
			})
			.subscribe(
				(res: any) => {
