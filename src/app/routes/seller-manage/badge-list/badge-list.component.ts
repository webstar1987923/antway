...
export class BadgeListComponent implements OnInit, OnDestroy {
	...
	getData() {
		...
		this.comSrv.getExpoBadgeOverview({ ...this.queryParams }).subscribe(
			(res: any) => {
				this.blkSrv.setBlockStatus(false);
				console.log('Badge overview list', res);
	...
	openView(record: any = {}) {
		this.modal
			.create(BadgeViewComponent, { data: { record } }, { size: 1240 })
			.subscribe((res) => {
				this.getData();
			});
	}
